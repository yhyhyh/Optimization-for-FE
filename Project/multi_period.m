function  multi_period
    ...
    % Try different gamma for testing
    for gamma = 0.1:0.1:0.9
        ...
        for i = 1:length(rebalance_dates)
            ...
            if (nearest>last_nearest)
                ...
                % [x0,x,cvar] = optimize_cvar(mu0,mu,V,xx0,xx,trans_cost,0);
                [x0, x, cvar] = cvx_cvar(scenario,ones(size(scenario,1),1),...
                    0.95,mean(mu),mu,mu0);
                cost = 0;
                % Do the second step of maximizing the utility function
                [x0, x] = cvx_utilfunc(scenario,ones(size(scenario,1),1),...
                    0.95,wealth/10000,mu,mu0,cvar*1.5,gamma);
        ...
        fprintf('Gamma: %f, return: %f, s.d.: %f\n',gamma,wealth,...
    ...
    % The code below is the MVO strategy same as homework 2
    for i = 1:length(rebalance_dates)

...
for i in range(0, 10):
    ...
    threshold_mom.loc[:, decile] = np.nanpercentile(df[MOM[1]], q=10.0*i, axis=1)

exist_return = df[fut_ret[1]].notnull()
# criterion_mom = df[MOM[1]] >= threshold_mom  # cannot broadcast this way
for i in range(0, 10):
    decile = column_deciles[i]
    select_mom = df[MOM[1]].apply(lambda x: x >= threshold_mom.loc[:, decile], axis=0)
...
# nan future return stock cannot be used for back test
    ...
    select = select_mom & exist_return # & select_size
